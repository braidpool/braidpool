...
function generateMockBeads() {
  const beads = [];
  const cohorts = 8; // Number of cohorts
  const beadsPerCohort = 6; // Beads per cohort
    ...
    for (let j = 0; j < beadsPerCohort; j++) {
      ...
      if (i > 0) {
        ...
        if (prevCohortBeads.length > 0) {
          const numParents = Math.min(Math.floor(Math.random() * 2) + 1, prevCohortBeads.length);
          const shuffled = prevCohortBeads.sort(() => 0.5 - Math.random());
          shuffled.slice(0, numParents).forEach((b) => parents.add(b.id));
        }
      }

      // If no parents were assigned and this isn't the first cohort, add the block as parent
      if (parents.size === 0 && i > 0) {
        parents.add(`block${i}`);
      }
  ...
  const beadMap = new Map(beads.map((bead) => [bead.id, bead]));
  beads.forEach((bead) => {
