...
\begin{abstract}
  ...
  on these two proposals. A DAG of shares is used to compute rewards,
  and mining rewards are paid out by an anonymous hub communicating
  with the miners using I2P. Using the payment channels construction,
  neither the hub nor the miners can cheat. We show that our approach
  is incentives compatible and describe how the hub maintains its
  anonymity to resist DDoS attacks.
\end{abstract}
...
address the problem where smaller pools can continue to get rewarded
for their PoW, without an increase in variance with increase in global
hash rate.
...
\item Allow miners to select transactions they want to mine, with no
  potential for a censor to deny them payouts.
...
and~\cite{spectre} use a DAG for faster block times and focus on
changing the consensus layer of bitcoin. These proposals allow for
miners to produce shares that have conflicting transactions and then
apply rules to find a set of transactions acceptable at various cuts
of the DAG.\

Braiding the blockchain proposal~\cite{mcelrath:variance} shows how
smaller more frequent blocks can form a DAG of blocks, with each block
pointing to one or more one previous blocks. Blocks in the braid,
called beads, can have transactions repeated in different blocks. The
proposal describes how duplicate and double spend transactions can be
resolved to reach a decision on the state of the ledger at any cut of
the DAG.\@
...
DEXes for executing futures contracts, but and these are initial ideas
and we will evaluted them in the future for incorporating them into
our proposal once they have been developed further.
...
from coinbase transactions. The hub locks in BTC for each miner by
opening payment channels for each of the miners. The construction
shows how both miners and hubs can not cheat and how the funders of
the hub earns rewards for funding the payment channels.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.5\textwidth]{work-share}
...
\begin{table}
  ...
  \begin{tabular}{ |l|l| }
    \hline
    \multicolumn{2}{|c|}{\textsc{Share}} \\
    \hline
    Blockhash & bitcoin block hash for \textsc{work} \\
    \hline
    $list<$ \textsc{share} $>$ & List of shares referenced by the \textsc{share}\\
    \hline
    Difficulty & claimed difficulty miner is using \\
    \hline
    PubKeys & The pubkeys used by the hub to for channel management \\
    \hline
    I2P destination & The I2P destination for the hub to contact the miner \\
    \hline
  \end{tabular}
  \caption{The structure of \textsc{share}s broadcast by miners.}\label{table:share}
\end{table}


Table~\ref{table:share} shows the structure of shares broadcast by the
miner. Each share includes the blockhash of the \textsc{work} the
miner is working on, along with the current target difficulty used by
the miner, the pubkey that the hub will use to create channels for the
miner, and finally an I2P destination address for the hub to reach the
miner.
...
particpating in the pool, $a$, $b$ and $c$. The nodes in the DAG are
...
rewarded for their work. We identify this degenerative case as
...
bitcoin block the reward is not shared with any other miner as $a_3$
does not include any references to shares from other miners.
...
A reward function is defined to as incentive compatible if every
miner's best response strategy reports full solutions
...
pool scale up without losing blockspace. The miners therefore avoid
...
miner only after it has contributed enough shares. This could be a
...
Table~\ref{table:coinbase} can now be spent by one of the following
branches:
...
We use I2P's tunnels to avoid compromising the anonymity by setting up
separate inbound and outbound tunnels, as required by I2P. According
to the I2P documentation~\cite{i2p-tech-intro}:
...
messages. The hub inturn uses different destination addresses when
contacting each hub, further strengthening its anonymity and
