...
use. Our initial suggestion is that miners use 10 bits from the extra
nonce field to include the multiplier they are using between the block
difficulty and the share difficulty. If the block difficulty is $D$,
and the share difficulty is $d$, then miners use the first 10 bits
from the extra nonce to encode the ratio $D/d$.
