...
\begin{abstract}
  ...
  blockspace with the number of miners on p2pool. Building a directed
  acyclic graph (DAG) of miner's shares and the use of payment
  channels are two proposals trying to alleviate these problems faced
  by p2pool. In this paper, we present a unified solution that uses a
  DAG to track miners shares and uses payments channels to reward
  miners. The shares calculation is verified by all participants of
  the pool, and the rewards are paid out by an anonymous hub
  communicating over I2P. Using the payment channels construction,
...
rewards. This is shown in the analysis by
McElrath~\cite{mcelrath:variance}. Ethereum's inclusive
...
rewarded for their PoW without an increase in variance with increasing
in global hash rate.
...
different blocks. The proposal describes how repeated and double spend
...
swap TerraHash Coins for BTC. The problems with the atomic swap
approach is the need for $O(n)$ block space as we need two
transactions to execute a swap. Burning THCoins for BTC requires a
...
executing futures contracts, but that is still in very nascent stage
of development and we don't go down that route here.
