...
const BraidVisualization: React.FC<BraidVisualizationProps> = ({
  ...
  useEffect(() => {
    if (!svgRef.current || !data?.nodes?.length) {
      console.log('⚠️ Cannot render visualization - missing ref or data:', {
...
const createDagLayout = (
  ...
  if (!data.cohorts || data.cohorts.length === 0) {
    console.warn('⚠️ No cohorts found in data, creating a fallback layout');
    ...
    data.nodes.forEach((node) => {
      ...
      const y =
        (row + 1) * (height / (Math.ceil(data.nodes.length / gridCols) + 1));
  ...
  const maxCohortSize = Math.max(...cohortSizes, 1); // Ensure at least 1 to avoid division by zero
  ...
  // Make sure all nodes have positions, even if they weren't in any cohort
  data.nodes.forEach((node) => {
    if (!nodePositions.has(node.id)) {
      console.warn(
        `⚠️ Node ${node.id} was not in any cohort, positioning at right edge`
      );
