      ...
      >
        ...
        >
          <defs>
            ...
            <linearGradient id="comparisonAreaGradient" x1="0%" y1="0%" x2="0%" y2="100%">
            ...
            <clipPath id="comparisonAreaClip">
              <motion.path d={comparisonAreaPath} initial={{ pathLength: 0 }} animate={chartControls} />
            </clipPath>
          </defs>
          ...
          {points.length > 0 &&
            points.map((point, i) => {
              ...
              if (timeRange === "year" && isMonthStart) {
                showGridLine = true
              } else if (timeRange === "quarter" && (isMonthStart || isWeekStart)) {
                showGridLine = true
              } else if ((timeRange === "month" || timeRange === "week") && isWeekStart) {
                showGridLine = true
              }
              return showGridLine ? (
                <motion.line
            ...
            <motion.g key={i} onHoverStart={() => setActivePoint(i)} onHoverEnd={() => setActivePoint(null)}>
              {point.trend !== "neutral" && (
                <motion.circle
                  ...
                  cy={point.y - 20}
                  r={12}
                  fill={point.trend === "up" ? "rgba(16, 185, 129, 0.2)" : "rgba(239, 68, 68, 0.2)"}
                  initial={{ scale: 0 }}
              ...
              <motion.circle
                ...
                cy={point.y}
                r={isHovered ? 12 : 8}
                fill={point.trend === "up" ? "rgba(16, 185, 129, 0.3)" : "rgba(59, 130, 246, 0.3)"}
                filter="url(#softGlow)"
                animate={{
              ...
              <motion.circle
                ...
                filter="url(#glow)"
                animate={{
                  y: isHovered ? [point.y - 5, point.y + 5, point.y - 5] : [point.y - 2, point.y + 2, point.y - 2],
                  scale: activePoint === i ? 1.5 : isHovered ? [1, 1.2, 1] : [1, 1.1, 1],
                }}
                transition={{
                  duration: 3,
                  repeat: Number.POSITIVE_INFINITY,
              ...
              {activePoint === i && (
                ...
                >
                  {point.trend === "up" ? (
                    <ArrowUpRight x={point.x - 6} y={point.y - 26} size={12} className="text-emerald-500" />
                  ) : (
                    <ArrowDownRight x={point.x - 6} y={point.y - 26} size={12} className="text-red-500" />
                  )}
                </motion.g>
              )}
              {activePoint === i && (
          ...
          {comparisonPath && (
            <g transform="translate(20, 20)">
              ...
              <rect y="25" width="15" height="15" fill="url(#comparisonGradient)" rx="2" />
