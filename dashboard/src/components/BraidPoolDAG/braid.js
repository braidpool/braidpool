...
function makeDag(hashedParents, beadWork = null, description = null) {
  ...
  dag.beadWork = beadWork
    ? new Map(beadWork)
    : new Map([...parents.keys()].map((b) => [b, FIXED_BEAD_WORK])); // Map<number, number>
  dag.work = descendantWork(parents, dag.children, dag.beadWork, dag.cohorts); // Map<number, number>
  dag.highestWorkPath = highestWorkPath(
    parents,
    dag.children,
    dag.beadWork,
    dag.work
...
function geneses(parents) {
  ...
  for (const [b, p] of parents.entries()) {
    if (!p || p.size === 0) {
      // Check if p exists and is empty
      retval.add(b);
    }
  }
  return retval;
...
function generation(beads, relationMap) {
  ...
  for (const b of beads) {
    ...
    if (relatedNodes) {
      // Check if the bead exists in the map and has related nodes
  ...
  while (workStack.length > 0) {
    ...
    if (isProcessed) {
      ...
      for (const p of currentParents) {
        ...
        if (parentAncestors) {
          // Check if parent's ancestors are calculated
          ...
          console.error(
            `Consistency error: Ancestor info for parent ${p} of bead ${current} not found.`
          );
...
function cohortHead(cohort, parents, children = null) {
  ...
  const externalParents = new Set(
    [...cohortParents].filter((p) => !cohort.has(p))
  ); // Step 2: Parents outside cohort
  ...
  if (headCandidate.size > 0) {
    // Only check intersection if headCandidate is not empty
  ...
  for (const b of beads) {
    ...
    if (parents.has(b)) {
      ...
      for (const p of beadParents) {
        if (beads.has(p)) {
          // Only include parents that are also in the bead set
...
function descendantWork(
  ...
  for (const c of revCohorts) {
    ...
    for (const b of c) {
      ...
      returnValue.set(
        b,
        previousWork + beadWork.get(b) + sumOfSubDescendantWork
      );
    }
    for (const b of c) {
...
function workSortComparator(parents, children = null, beadWork = null) {
  ...
  beadWork = beadWork
    ? beadWork
    : new Map([...parents.keys()].map((b) => [b, FIXED_BEAD_WORK]));
  ...
  while (!dagTips.has(hwPath[hwPath.length - 1])) {
    ...
    if (currentChildren.size === 0) {
      ...
      if (!dagTips.has(currentBead)) {
        console.warn(
          `Highest work path reached bead ${currentBead} which is not a tip but has no children.`
        );
      }
    ...
    if (sortedChildren.length === 0) {
      ...
      console.warn(
        `Highest work path: Bead ${currentBead} has children set, but sorted result is empty.`
      );
      break;
    }
    ...
    if (hwPath.length > parents.size * 2) {
      // Heuristic limit
      console.error(
        'Highest work path calculation exceeded graph size * 2. Stopping to prevent potential infinite loop.'
      );
      break;
    }
  }
...
function layout(
  ...
  function intersection(x1, y1, x2, y2, x3, y3) {
    ...
    if (Math.abs(crossProduct) > 1e-9) {
      // Javascript can't do integer math
      return false;
    }
  ...
  const allRelevantBeads = new Set([
    ...cohort,
    ...(previousCohortTipsPos ? previousCohortTipsPos.keys() : []),
  ]);
  for (const bead of allRelevantBeads) {
    ...
    if (work === undefined || work === null) {
      ...
      if (allParents.has(bead) || cohort.has(bead)) {
        // Check if it's a valid bead in the context
