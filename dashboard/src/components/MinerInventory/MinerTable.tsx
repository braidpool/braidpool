...
const MinerTable: React.FC<MinerTableProps> = ({
  ...
  return (
    <div className="w-full overflow-x-auto">
      ...
      >
        <div
          className={`grid ${hasAlerts ? 'grid-cols-9' : 'grid-cols-8'} gap-4 px-4 py-3 text-xs uppercase tracking-wide text-textSecondary border-b border-border`}
        >
          <div>Model</div>
          <div>Hashrate</div>
          <div>Efficiency</div>
        ...
        <div className="rounded-2xl   shadow-md p-4">
          {miners.map((miner) => {
            ...
            return (
              <div
                key={miner.id}
                className={`grid ${hasAlerts ? 'grid-cols-9' : 'grid-cols-8'} gap-4 px-4 py-3 text-sm text-textSecondary items-center`}
              >
