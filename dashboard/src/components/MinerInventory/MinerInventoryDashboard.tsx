...
const MinerInventoryDashboard = () => {
  ...
  const fetchMinerData = async (ip: string): Promise<Miner | null> => {
    try {
      ...
      if (!response.ok || !responseData.success) return null;
  ...
  return (
    <div className="min-h-screen w-full px-4 py-6 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-6 px-4 py-3 bg-red-950 text-red-400 text-sm rounded-lg border border-red-900">
            {error}
          </div>
        )}
          ...
          <>
            ...
            <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex border border-gray-700 rounded-full p-1 gap-1 w-fit">
                {(
                  [
                    { key: 'all', label: 'All', count: totalMiners },
                    { key: 'online', label: 'Online', count: onlineMiners },
                    { key: 'warning', label: 'Warning', count: warningMiners },
                    { key: 'offline', label: 'Offline', count: offlineMiners },
                  ] as const
                ).map(({ key, label, count }) => (
                  <button
                    key={key}
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={clearSearch}
                  className="px-3 py-1.5 text-sm text-gray-400 rounded-lg border border-gray-700 hover:text-gray-200 transition"
                >
                  Clear
                </button>
                <select
                  ...
                  className="px-3 py-1.5 text-sm border border-gray-700 bg-gray-900 rounded-lg text-gray-300 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                >
