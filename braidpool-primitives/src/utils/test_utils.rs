...
fn generate_merkel_proof(merkel_tree: Vec<String>, curr_tx_id: String) -> Vec<(String, String)> {
    let reversed_curr_tx = reverse_hex_byte_order(&curr_tx_id);
    let mut sibling_hashes: Vec<(String, String)> = Vec::new();
    if merkel_tree.contains(&reversed_curr_tx) == false {
    } else {
        //dummy value
        let mut start_index = 1010;
        for index in (0..merkel_tree.len()) {
            if merkel_tree[index] == reversed_curr_tx {
                start_index = index;
                break;
            }
        }
        if start_index == 1010 {
            ...
            // sibling_hashes.push(merkel_tree[start_index].clone());
            if (start_index % 2 != 0) {
                //odd so will be in left the other sibling will be in right
                if (start_index + 1 <= merkel_tree.len() - 1) {
                    sibling_hashes.push((merkel_tree[start_index + 1].clone(), String::from("R")));
                    start_index = (start_index - 1) / 2;
                }
            } else if (start_index % 2 == 0) {
                //even so in right the other sibling will be in left
                if (start_index - 1 >= 0) {
                    sibling_hashes.push((merkel_tree[start_index - 1].clone(), String::from("L")));
                    start_index = (start_index - 2) / 2;
                }
            }
            while (start_index > 0) {
                let curr_parent_hash = &merkel_tree[start_index];
                if (start_index % 2 != 0) {
                    //odd so will be in left the other sibling will be in right
                    if (start_index + 1 <= merkel_tree.len() - 1) {
                        sibling_hashes
                            .push((merkel_tree[start_index + 1].clone(), String::from("R")));
                        start_index = (start_index - 1) / 2;
                    }
                } else if (start_index % 2 == 0) {
                    //even so in right the other sibling will be in left
                    if (start_index - 1 >= 0) {
                        sibling_hashes
                            .push((merkel_tree[start_index - 1].clone(), String::from("L")));
                        start_index = (start_index - 2) / 2;
                    }
                }
            }
...
fn merkle_tree(leaves: Vec<String>) -> (String, Vec<String>) {
    let mut merkel_tree: Vec<String> = Vec::new();
    ...
    let mut layer: Vec<String> = leaves
        ...
        .collect();
    let mut reference_layer = layer.clone();
    while layer.len() > 1 {
        if layer.len() % 2 != 0 {
        ...
        for i in (0..layer.len()).step_by(2) {
            ...
            let combined_reference = combined.clone();
            // merkel_tree.push(hash_data(&combined_reference));
            new_layer.push(hash_data(&combined));
        }
        let new_layer_reference = new_layer.clone();
        levels.push(new_layer_reference);
        layer = new_layer;
    }
    levels.reverse();
    for level in levels {
        for val in level {
            merkel_tree.push(val);
        }
    }
    if reference_layer.len() % 2 != 0 {
        reference_layer.push(reference_layer.last().unwrap().clone());
    }

    for tx in reference_layer {
        merkel_tree.push(tx);
    }
    (layer[0].clone(), merkel_tree)
}
pub fn create_dummy_merkle_path_proof(
    transaction_id: Txid,
    siblings: Vec<([u8; 32], String)>,
    is_coinbase: bool,
) -> MerklePathProof {
    let mut converted_vector: Vec<(TxMerkleNode, String)> = Vec::new();
    for sibling in 0..siblings.len() {
        converted_vector.push((
            TxMerkleNode::from_byte_array(siblings[sibling].0),
            siblings[sibling].clone().1,
        ));
    }
    if is_coinbase {
...
pub fn create_merkel_tree(transactions: Vec<Transaction>) -> Vec<String> {
    let mut merkel_tree: Vec<String> = Vec::new();
...
pub fn create_test_bead(
    ...
    let merkle_structure = merkle_tree(total_txs);
    //merkel node to be added inside the block header
    let merkel_node = merkle_structure.0;
    //generating merkle tree for the intaked transactions
    let merkel_tree = merkle_structure.1;
    //getting the corresponding merkel proof for coinbase
    let merkel_proof_coinbase =
        generate_merkel_proof(merkel_tree.clone(), coinbase_txid.to_string());
    //getting the corresponding merkel proof for payout transaction
    let merkel_proof_payout = generate_merkel_proof(merkel_tree.clone(), payout_txid);
    let mut merkel_proof_payout_bytes: Vec<([u8; 32], String)> = Vec::new();
    let mut merkel_proof_coinbase_bytes: Vec<([u8; 32], String)> = Vec::new();
    //converting the hex string to bytes for siblings txids for coinbase tx

    for coinbase_sibling in merkel_proof_coinbase {
        let mut bytes = [0u8; 32];
        let result = hex::decode_to_slice(coinbase_sibling.0, &mut bytes);
        match result {
            Ok(_) => {
                merkel_proof_coinbase_bytes.push((bytes, coinbase_sibling.1));
                println!("Successfully decoded the corresponding sibling");
            }
            Err(error) => {
                println!("An error occurred while decoding --- {:?}", error);
            }
        }
    }
    //converting the hex string to bytes for siblings txids for payout
    for payout_sibling in merkel_proof_payout {
        let mut bytes = [0u8; 32];
        let result = hex::decode_to_slice(payout_sibling.0, &mut bytes);
        match result {
            Ok(_) => {
                merkel_proof_payout_bytes.push((bytes, payout_sibling.1));
                println!("Successfully decoded the corresponding sibling");
            }
            Err(error) => {
                println!("An error occurred while decoding --- {:?}", error);
    ...
    let payout_transaction_with_merkel_path: TransactionWithMerklePath = (
        payout_tx_reference,
        create_dummy_merkle_path_proof(payout_tx_id, merkel_proof_payout_bytes, false),
    );
    let coinbase_transaction_with_merkel_path: TransactionWithMerklePath = (
        coinbase_transaction,
        create_dummy_merkle_path_proof(coinbase_txid, merkel_proof_coinbase_bytes, true),
    );
    ...
    //bead hash value taking random 32 byte hash but same as that of the block header bytes for
    //TODO : A serializing trait for each inorder for combining the raw beads,raw braids etc.
