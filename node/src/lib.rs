...
/// Get the current active template ID (the latest one in the cache)
pub async fn get_latest_template_id(
    template_cache: &tokio::sync::Mutex<HashMap<String, Arc<crate::ipc::client::BlockTemplate>>>,
) -> Option<String> {
    let cache = template_cache.lock().await;

    // Find the highest template_id (since they increment monotonically)
    cache
        .keys()
        .filter_map(|k| k.parse::<u64>().ok().map(|id| (id, k.clone())))
        .max_by_key(|(id, _)| *id)
        .map(|(_, k)| k)
}
...
pub async fn ipc_template_consumer(
    ...
    const MAX_CACHED_TEMPLATES: usize = 500;
