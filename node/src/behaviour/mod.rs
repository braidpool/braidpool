...
pub fn identify_protocol(network: &str) -> String {
    format!("/braidpool/{network}/identify/1.0.0")
}
pub fn bead_sync_protocol(network: &str) -> StreamProtocol {
    StreamProtocol::try_from_owned(format!("/braidpool/{network}/bead-sync/1.0.0"))
        .expect("network name produces invalid StreamProtocol")
}
pub fn braidpool_topic(network: &str) -> String {
    format!("braidpool_channel/{network}")
}
...
impl BraidPoolBehaviour {
    pub fn new(local_key: &Keypair, network: &str) -> Result<BraidPoolBehaviour, Box<dyn Error>> {
